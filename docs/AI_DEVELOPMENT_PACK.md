...
- Current verified baseline after T43 is 141 passing tests.
- RAG is ON.
- Tool-Use is ON.
- Agentic and Planning profiles are OFF.
- The product is local-first and single-operator.
- The current implementation wave started at T39 and changes the product from manual/snapshot-first collection to live-source-first collection with exports as fallback. The next task is T44.
