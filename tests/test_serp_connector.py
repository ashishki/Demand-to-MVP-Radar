...
def test_serp_connector_cache_only_uses_fixture_without_credentials(
    tmp_path,
    monkeypatch,
    capsys,
) -> None:
    monkeypatch.delenv("SERPAPI_API_KEY", raising=False)
    config_path = _write_config(tmp_path, cache_only=True)
    ...
    assert output["source_counts"]["serp-live"] == 2
    assert output["error_counts"]["serp-live"] == 0
    assert output["source_counts"]["source_modes"]["serp-live"] == "cache_only"
    assert "serp-live" not in output["source_errors"]


def test_serp_connector_dry_run_skips_live_without_credentials(
    tmp_path,
    monkeypatch,
    capsys,
) -> None:
    monkeypatch.delenv("SERPAPI_API_KEY", raising=False)
    config_path = _write_config(tmp_path, dry_run=True, fixture_path=None)
    ...
    assert output["source_counts"]["serp-live"] == 0
    assert output["error_counts"]["serp-live"] == 0
    assert output["source_counts"]["source_modes"]["serp-live"] == "dry_run"
    assert "serp-live" not in output["source_errors"]
...
def _write_config(
    ...
    *,
    ...
    dry_run: bool = False,
    fixture_path: Path | None = FIXTURE_DIR / "search_results.json",
) -> Path:
    source = {
    ...
    if fixture_path is not None:
        source["fixture_path"] = str(fixture_path)
